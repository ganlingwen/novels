...
的四项合并决策又新增 4 SFT / 4 DPO，累计为 182 SFT / 240 DPO。
PR #141 新增 4 SFT / 4 DPO，PR #142 新增 15 SFT / 16 DPO，
使 PR #143 沉淀前的存量达到 **201 SFT / 260 DPO**。

PR #143 的[早餐与出门编辑记录](real/pr143_ch4_breakfast_departure.json)
新增 **5 SFT / 2 DPO**；两条 DPO 来自作者明确要求调整起身顺序及澄清指代，
其余三条仅记录已接受修订，不自动把原文标成负例。暂缓的病情与治疗进度调整不计入。
当前 `real/` 共 **334 条编辑记录、81 个 JSON 文件**，loader 输出为
**206 SFT / 262 DPO**。编辑记录数、SFT 样本数与 DPO 配对数是不同口径。
